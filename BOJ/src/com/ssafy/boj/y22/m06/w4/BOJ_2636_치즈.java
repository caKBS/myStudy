...
public class BOJ_2636_치즈 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		// 치즈 외부 칸을 스캔하면서 치즈 외부칸과 닿아 있는 애들을 삭제시킨다.
		...
		while (true && left > 0) {
			...
			DFS(0, 0, check, mark);
			// 제거 후 남은 치즈 개수가 들어간다.
			int aftDel = delete(mark);
			// 제거 후 남은 치즈개수가 0이면 즉각 게임 종료
			if (aftDel == 0) {
				break;
				// 아직 치즈가 남아있으면 남은 개수 저장
				// 이게 나중에 할때 break할때
				// 모두 녹기 한시간전 남아있는 치즈칸수가 된다.
			} else {
				left = aftDel;
			}
	...
	public static int delete(boolean[][] mark) {
		int cnt = 0;
		for (int r = 0; r < R; r++) {
			for (int c = 0; c < C; c++) {
				if (mark[r][c]) {
	...
	public static void DFS(int r, int c, boolean[][] check, boolean[][] mark) {
		check[r][c] = true;
		...
		for (int i = 0; i < 4; i++) {
			...
			if (BC(nr, nc) && !mark[nr][nc] && board[nr][nc] == 1) {
				mark[nr][nc] = true;
			}
		}
