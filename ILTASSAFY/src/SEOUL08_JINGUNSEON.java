...
public class SEOUL08_JINGUNSEON {

	// 닉네임을 사용자에 맞게 변경해 주세요.
	static final String NICKNAME = "SEOUL08_JINGUNSEON";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				float width = 0;
				float height = 0;
				double[] distances = new double[NUMBER_OF_BALLS];
				
				for(int i = 1; i< NUMBER_OF_BALLS-1; i+=2) {
					if(balls[i][0] == -1 && balls[i][1] == -1) 
						continue;
					else {
					width = Math.abs(balls[i][0] - whiteBall_x);
					height = Math.abs(balls[i][1] - whiteBall_y);

					// distance: 두 점(좌표) 사이의 거리를 계산
					 distances[i] = Math.sqrt((width * width) + (height * height));
					}
					}
				...
				if(distances[1] == 0 && distances[2] == 0 && distances[3] == 0 && distances[4] == 0) {
					targetBall = 5;
				}
			
				// targetBall_x, targetBall_y: 목적구의 X, Y좌표를 나타내기 위해 사용한 변수
				float targetBall_x = balls[targetBall][0];
				float targetBall_y = balls[targetBall][1];
				
				width = Math.abs(balls[targetBall][0] - whiteBall_x);
				height = Math.abs(balls[targetBall][1] - whiteBall_y);
